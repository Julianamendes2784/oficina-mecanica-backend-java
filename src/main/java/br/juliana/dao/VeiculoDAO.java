...
public class VeiculoDAO {
    ...
    private String descrever(Veiculo v) {
        return String.format("Placa: %s | Modelo: %s | Ano: %d | Cor: %s | KM: %d | Cliente ID: %d",
                v.getPlaca(), v.getModelo(), v.getAno(), v.getCor(), v.getQuilometragem(), v.getClienteId());
    }
    ...
    // 1. CADASTRAR
    public void cadastrar(Veiculo veiculo, int usuarioId) {
        ...
        try (Connection conn = FabricaConexao.getConnection();
            ...
            stmt.setString(1, veiculo.getPlaca().trim().toUpperCase());
            ...
            try (ResultSet rs = stmt.getGeneratedKeys()) {
                if (rs.next()) {
                    ...
                    logDAO.registrar(new LogAuditoria(usuarioId, "veiculos", (int) novoId, "INSERT",
                            "Veículo cadastrado. " + descrever(veiculo)));
                }
            }
    ...
    public void cadastrar(Veiculo veiculo) { cadastrar(veiculo, 0); }
    ...
    // 3. ATUALIZAR
    public void atualizar(Veiculo veiculo, int usuarioId) {
        Veiculo antes = buscarPorId(veiculo.getId());
        ...
        try (Connection conn = FabricaConexao.getConnection();
            ...
            if (linhas > 0) {
                String descricao = "ANTES: " + (antes != null ? descrever(antes) : "N/A")
                        + " || DEPOIS: " + descrever(veiculo);
                logDAO.registrar(new LogAuditoria(usuarioId, "veiculos", (int) (long) veiculo.getId(), "UPDATE", descricao));
    ...
    public void atualizar(Veiculo veiculo) { atualizar(veiculo, 0); }

    // 4. EXCLUIR
    public void excluir(Long id, int usuarioId) {
        Veiculo antes = buscarPorId(id);
        ...
        try (Connection conn = FabricaConexao.getConnection();
            ...
            int linhas = stmt.executeUpdate();
            if (linhas > 0) {
                logDAO.registrar(new LogAuditoria(usuarioId, "veiculos", (int) (long) id, "DELETE",
                        "Veículo excluído. " + (antes != null ? descrever(antes) : "ID: " + id)));
